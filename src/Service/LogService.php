    ...
    public function debug(
        ...
        if (!$ignoreRedis && ($this->data['channel'] === false || !$this->contextService->debugMode())) {
            $this->sharedLogCache->addLog([
                ...
                'tenantName' => $user->tenant->alias ?? 'none',
                'tenant' => $user->tenant->id ?? 'none',
                'auth' => $user->id ?? 'none',
                'apiVersion' => $this->contextService->apiVersion,
            ]);
            ...
            $data = [
                ...
                'logging.googleapis.com/labels' => [
                    ...
                    ...[
                        ...
                        'tenantName' => $user->tenant->id ?? 'none',
                        'auth' => $user->id ?? 'none',
                        'apiVersion' => $this->contextService->apiVersion,
                    ],
                ],
        ...
        foreach ($logs as $log) {
            $data = [
                ...
                'logging.googleapis.com/labels' => [...($log['labels'] ?? []), ...[
                    'app' => $log['project'],
                    'env' => $this->contextService->env(),
                    'tenantName' => $log['tenantName'] ?? 'none',
